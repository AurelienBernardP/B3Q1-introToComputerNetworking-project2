...
class ControlChannel extends Thread {
    ...
    private void processRequest(String request) throws Exception {
        ...
        case "MDTM":
            ...
            try {
                String lastModified = VirtualFileSystem.getInstance().getFile(currentFolder, words[1])
                        .getLastModified();
        ...
        case "RNTO":
            ...
            controlResponse(new FTPCode().getMessage(250));
            break;
        case "LIST":// see current directory content, no arg( we dont have to handle the case where
                    // there is an arg)
            if (dataChannel != null) {
        ...
        case "PWD":// gives path of current directory, no arg
            controlResponse(
                    new FTPCode().getMessage(257) + VirtualFileSystem.getInstance().getPWD(currentFolder) + "\r\n");
            break;
        ...
        case "USER": // input user name, 1 arg, the user name
            requestUSER(words);
            break;
        case "PASS": // input password, 1 arg, the password(i think we have to decription it as we
                     // receive it cripted)
            requestPASS(words);
            break;
    ...
    private void requestPASV() {
        ...
        controlResponse(new FTPCode().getMessage(227) + " ("
                + socketControl.getLocalAddress().toString().replace('.', ',').replace("/", "") + ","
                + Integer.toString(dataPort[0]) + "," + Integer.toString(dataPort[1]) + ")\r\n");
    ...
    void requestPORT(String[] request) {
        ...
        int portClient = transitionClientPort(Integer.parseInt(interfaceClient[4]),
                                            Integer.parseInt(interfaceClient[5]));

        this.dataChannel = new DataChannel(this, 2046);
        dataChannel.startListening(socketControl.getInetAddress(), portClient);
        controlResponse(new FTPCode().getMessage(200));
